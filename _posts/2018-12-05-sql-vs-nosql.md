...
В данном посте будут относительно детально описаны реляционные и некоторые нереляционные БД. Читатель узнает как все начиналось в контексте баз данных, к чему пришло, чем отличаются NoSQL от реляционных конкурентов и, собственно, в чем их различия, props & cons.

## Реляционная модель и SQL
> Начинай с малого, не сойдешь за отсталого.  

Поэтому начнем с истории о том, как появились реляционки, что это такое и чем они так привлекательны. Всем просвященным или принципиально не читающим теорию и исторические сводки можно преспокойно скипнуть дальше.  
...
Вся эта движуха длилась до конца 60-х годов, когда Эдгар Кодд не сформулировал и изложил принципы *реляционной модели*.  
...
Итак, что такое *реляционная база данных* (РБД) - это БД, основанная на одноименной модели данных (РМД). Она заставляет нас структурировать данные так, что их можно строго формализовать математически и удобно ими манипулировать, а также максимально оптимизировать эти манипуляции (т.к. теперь мы имеем дело с математикой, то эффективность того или иного решения можно строго доказать).  
...
* Жесткость структуры данных, также есть сложность описания иерархических и сетевых связей;
...
Но погодите кричать "шах и мат nosql'ные хипстеры!"
